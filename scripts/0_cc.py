...
winCcPath = F'/mnt/d/5_Watershed/7_cc/{ws}'
...
for ccmod in ccmods:
    ...
    for ssp in os.listdir(ccmodPath):
        # if ssp in ['ssp126','ssp370']: ####################################### TO FILTER SSP
...
#.______________________
        data_pr = pd.read_csv(os.path.join(sspPath,'pr.csv'),index_col=0,parse_dates=True)
        data_tmax = pd.read_csv(os.path.join(sspPath,'tasmax.csv'),index_col=0,parse_dates=True)
        data_tmin = pd.read_csv(os.path.join(sspPath,'tasmin.csv'),index_col=0,parse_dates=True)

        data_pr = data_pr[cd_filt.index]
        data_pr = data_pr*86400/25.4 # inches
        data_tmax = data_tmax[cd_filt.index]
        data_tmax = ((data_tmax - 273.15) * 9/5) +32 # K to C to F
        data_tmin = data_tmin[cd_filt.index]
        data_tmin = ((data_tmin - 273.15) * 9/5) +32 # K to C to F
