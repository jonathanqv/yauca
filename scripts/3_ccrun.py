...
from run import run
from runutils import multirun_write
...
def running():
    ccBasePath = '../sim/cc'
    ccDataPath = '/mnt/c/Users/lrbk/Documents/CSM/Proyects/5_Watershed/4_MetDataProcessed/climate_change'
    mrBasePath = '../sim/multirun'
    ccPath = os.path.join(modRunPath,'7_cc',ws)
    ccOutPath = os.path.join(modRunPath,'8_cc_outputs',ws)
    ...
    #.____________________________
        ...
        for ccmod in ccmods:
            ...
            for ssp in os.listdir(ccmodPath): # SSP MODELS TO RUN
                # if ssp in ['ssp245','ssp585']: ############################################# TO TEST
    ...
    #._____________________________
                ...
                for vrb in variables:
                    # Opening base variable
                    vrbPath = os.path.join(baseDataPath,F"{vrb}.day")
                    dfVrb = gsflow.prms.PrmsDay(vrbPath).dataframe
                    # Opening climate change variable                    
                    ccVrbPath = os.path.join(ccPath,F"{ssp}_{ccmod}_{vrb}.parquet.gz")
                    ccVrb = pd.read_parquet(ccVrbPath)
                    # New datadf
                    dfnew = pd.concat([dfVrb,ccVrb])
                    # Writing df
                    ...
                    dfnew = dfnew[cols]
                    multirun_write(os.path.join(baseDataPath,F'{vrb}_cc.cbh'),dfnew,var=vrb)
    # Creating multirun
    #._____________________________
